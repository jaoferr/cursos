...
for i=1:num_labels
    A = -yvec(:,i)'*log(a3(:,i));
    B = (1-yvec(:,i))'*log(1-a3(:,i));
    C(i) = sum(A-B);
endfor
D = sum(Theta1(:,2:end)'.^2);
E = sum(D);
F = sum(Theta2(:,2:end)'.^2);
G = sum(F);
sd = size(D);
se = size(E);
sf = size(F);
...
#derivatives
Theta1_grad = (1/m)*(delta2(:,2:end)'*A1) + ((lambda/m)*Theta1);
Theta1_grad_temp = (1/m)*(delta2(:,2:end)'*A1);
Theta1_grad(:,1) = Theta1_grad_temp(:,1);

Theta2_grad = (1/m)*(delta3'*A2) + ((lambda/m)*Theta2);
Theta2_grad_temp = (1/m)*(delta3'*A2);
